...
from .models import Cart, CartItem, CartItemNew, Coupon, Order, OrderItem, Payment, RazorpayPayment
...
def cart(request):
    ...
    if cart_items:
        ...
        # Discount (if coupon applied, adjust as needed)
        if hasattr(cart, 'coupon') and cart.coupon:
            totals['discount'] = cart.coupon.discount_amount(totals['subtotal'])
...
def apply_coupon(request):
    """
    View to apply a coupon code to the cart
    """
    if request.method == 'POST':
...
def checkout(request):
    ...
    if coupon_id:
        try:
            ...
            if coupon.is_valid and totals['subtotal'] >= coupon.min_purchase:
